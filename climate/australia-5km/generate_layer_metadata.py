...
def gen_metadatajson(template, datadir, swiftcontainer, category, resolution):
...
def main(argv):
    ...
    for dataset in glob.glob(os.path.join(srcdir, '*')):
        gen_metadatajson(JSON_TEMPLATE, dataset, SWIFT_CONTAINER, category, resolution)

    for genre in (u"Climate", u"FutureClimate"):
